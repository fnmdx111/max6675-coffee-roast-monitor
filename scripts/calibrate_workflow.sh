...
"$PYTHON_BIN" scripts/calibrate_capture.py --reference-c 0 --duration-sec 30 --interval-sec 0.5 --output "$OUT"
...
"$PYTHON_BIN" scripts/calibrate_capture.py --reference-c 100 --duration-sec 30 --interval-sec 0.5 --output "$OUT"

echo ""
echo "Step 3/3: Applying calibration into config.json"
"$PYTHON_BIN" scripts/calibrate_apply.py --input "$OUT" --config config.json
